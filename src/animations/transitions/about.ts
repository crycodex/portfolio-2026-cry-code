...
const setupInAnimation = (about: HTMLElement) => {
  inMM = createMatchMedia((_context, { isMobile, isLandscape }) => {
    ...
    tl.fromTo(
      room.chairScrollRotation,
      { x: 0, y: 0, z: 0 },
      { x: -0.9, y: -1.1, z: -1.3, duration: 0.65, ease: "power3.out" },
      0,
    );

    tl.fromTo(avatar.tIdleIntensity, { value: 0 }, { value: 1, duration: 0.3, ease: "power2.inOut" }, 0);
